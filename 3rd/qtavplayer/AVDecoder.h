...
private :
    ...
    bool mIsOpenAudioCodec = false;
    bool mIsOpenVideoCodec = false;
    bool mHasSubtitle;


    AVFrame *mAudioFrame = nullptr;
    AVFrame *reciveFrame = nullptr;
    AVFrame *mHWFrame = nullptr; //硬解BUFFER
    QVector<RenderItem *> mRenderList; //渲染队列
    int maxRenderListSize = 10; //渲染队列最大数量
    QMutex mRenderListMutex; //队列操作锁
    int mRenderListCount;//队列数量
    RenderItem *mLastRenderItem = nullptr; //上一次的渲染对象


    int mFrameIndex = 0; //使用的

    QString mFilename;

    int mFrameFinished;
    bool mIsReadFinish = false;
    bool mIsVideoLoadedCompleted = true; //视频是否加载完成
    bool mIsAudioLoadedCompleted = true; //音频是否加载完成
    bool mIsSeek = false; //是否进行了拖动处理
    bool mIsSeekd = true; //当所有元素拖动完成后，些属性为true
    bool mIsAudioSeeked = true; // 音频是否拖动完成
    bool mIsVideoSeeked = true; //视频是否拖动完成
    bool mIsSubtitleSeeked = true; //
    int mSeekTime = -1; //拖动的时间
    bool mIsInit = false;
    bool mIsNeedCallRenderFirstFrame = true;

    bool mIsAudioPlayed = true; // 音频是否播放完成
    bool mIsVideoPlayed = true;  //视频是否播放完成
    bool mIsSubtitlePlayed = true;

    /** 视频的旋转角度，由于手机录出来的视频带有旋转度数 */
    int mRotate = 0;

    bool mIsVideoBuffered = false;
    bool mIsAudioBuffered = false;
    bool mIsSubtitleBuffered = false;
    ...
    qint64 mLastPos = 0;
    /** 播放速率 */
    int mPlayRate = AVDefine::AVPlaySpeedRate_Normal;
    /** 实际的播放比率 */
    float mRealPlayRate = 1.0f;
    /** 源采样率 */
    int mSourceSampleRate = 8000;
    /** 源生音频格式 */
    ...
    bool mUseHw = false;
    /** 是否是伴唱 */
    bool mIsAccompany = false;
    /// 是否是直播
    bool mIsLiving = false;
    ///声音滤镜是否初使化成功
    bool mIsAudioFilterInited = false;


    AVFilterContext *mAudioBufferSinkCtx = nullptr;
    AVFilterContext *mAudioBufferSrcCtx = nullptr;
    AVFilterGraph *mAudioFilterGraph = nullptr;
    AVFrame *mAudioFilterFrame = nullptr;
    QMutex mAudioFilterMutex;

    AVFilterInOut *mAudioOutputs = nullptr;
    AVFilterInOut *mAudioInputs = nullptr;

    AVDefine::AVChannelLayout mAudioChannelLayout = AVDefine::AVChannelLayout_Auto;
